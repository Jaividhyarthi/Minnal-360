...
function IntroVideo({ onDone }: { onDone: () => void }) {
  return <div style={{ position: 'fixed', inset: 0, background: '#000', zIndex: 100 }}><video src={`${import.meta.env.BASE_URL}grid_intro.mp4`} autoPlay muted playsInline onEnded={onDone} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /><button onClick={onDone} style={{ position: 'absolute', bottom: 24, right: 24, padding: '8px 16px', background: 'rgba(255,255,255,0.15)', color: '#fff', border: '1px solid rgba(255,255,255,0.4)', borderRadius: 6, cursor: 'pointer' }}>Skip</button></div>;
}
...
function App() {
  ...
  if (showIntro) return <IntroVideo onDone={() => setShowIntro(false)} />;
