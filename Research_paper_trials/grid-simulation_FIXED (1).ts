...
class GridSimulation {
  ...
  constructor() {
    ...
    for (let index = 0; index < 10; index += 1) {
      this.decisions.push(this.createDecision(AGENTS[index % AGENTS.length], index, undefined, index * 42));
    }
  ...
  private addDecision(
    ...
    this.decisions.unshift(
      // Live decisions use real creation time (backdateSeconds defaults to 0) --
      // tickCount is passed only as an id-disambiguator, never as a time offset.
  ...
  private createDecision(
    ...
    // backdateSeconds is ONLY for synthesizing plausible history at startup
    // (see constructor). Live decisions created via addDecision leave it at
    // the default 0, so `timestamp` reflects real creation time.
    const timestamp = new Date(Date.now() - backdateSeconds * 1000).toISOString();
    return {
